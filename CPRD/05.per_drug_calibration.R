...
## Standard drugs ----
rank1_optimal_standard <- analysis_standard %>%
...
rank1_optimal_standard_inje <- analysis_standard %>%
  rbind(analysis_oral, analysis_injectable %>% select(-split)) %>%
  cateval::get_best_drugs(
    rank = 1,
...
tolerance3_optimal_standard <- analysis_standard %>%
  rbind(analysis_oral, analysis_injectable %>% select(-split)) %>%
  cateval::get_best_drugs(
    tolerance = 3,
    column_names = paste0("pred.", c("DPP4", "GLP1", "SGLT2", "SU", "TZD")),
...
tolerance3_optimal_standard_inje <- analysis_standard %>%
  rbind(analysis_oral, analysis_injectable %>% select(-split)) %>%
  cateval::get_best_drugs(
    tolerance =3,
    column_names = paste0("pred.", c("DPP4", "GLP1", "SGLT2", "SU", "TZD", "Inje")),
...
overall_benefit_calibration_rank1_post_2019_summary <- cateval::compute_overall_benefit_performance(
...
overall_benefit_calibration_rank1_post_2019 <- cateval::compute_overall_benefit(
  data = analysis_standard %>% mutate(hba1c_group = ntile(prehba1c, 10)),
  drug_var = "drugclass",
  outcome_var = "posthba1cfinal",
  cal_groups = 10,
...
overall_benefit_calibration_tolerance3_post_2019 <- cateval::compute_overall_benefit(
  data = analysis_standard %>% mutate(hba1c_group = ntile(prehba1c, 10)),
  drug_var = "drugclass",
  outcome_var = "posthba1cfinal",
  cal_groups = 10,
...
plot_calibration_rank1 <- overall_benefit_calibration_rank1_post_2019 %>%
...
plot_calibration_tol3 <- overall_benefit_calibration_tolerance3_post_2019 %>%
  ggplot(aes(x = mean, y = coef, ymin = coef_low, ymax = coef_high)) +
  geom_vline(aes(xintercept = 0), colour = "black", linetype = "dashed") +
  geom_hline(aes(yintercept = 0), colour = "black", linetype = "dashed") +
  geom_abline(aes(intercept = 0, slope = 1), colour = "red") +
